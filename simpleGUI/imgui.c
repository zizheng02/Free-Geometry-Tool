...
gs_menu_color = {
	"White",      "Black",	"Blue",	    "White",   1 , // 填充
},

gs_button_color = {
	"White",      "Black",	"Cyan",	    "Black",   1 , // 填充
},

gs_textbox_color = {
	"White",      "Black",	"White",	    "Red",   1 , // 填充
};
...
{
	if( gs_menu_color.fillflag ) {
		mySetPenColor( gs_menu_color.label );
		drawRectangle(x, y, w, h, 0);
	}
	else
	{
...
int textbox(int id, double x, double y, double w, double h, char textbuf[], int buflen)
{
	...
	int line; // 行数 
	double height = h * line; 
	double textPosY = y + h / 2 - GetFontAscent() / 2;
	double cursorX = x + indent; // 光标x坐标 
	double cursorY = textPosY; // 光标y坐标 
	int currentN = len - toLeft; // 当前字符的下标 
	
	if (notInMenu(gs_UIState.mousex, gs_UIState.mousey) &&
		inBox(gs_UIState.mousex, gs_UIState.mousey, x, x + w, y - (line -1) * h, y - (line -1) * h + height) ) 
	{
	...
	// get line
	int n = 0;
	for (line = 0; n < len; line++)
	{
		MovePen(x + indent, textPosY - h * line);
		char tempt[2];
		tempt[1] = '\0';
		tempt[0] = textbuf[n];
		while (n < len && GetCurrentX() + TextStringWidth("|") + TextStringWidth(tempt) <= x + w)
		{
			if (GetCurrentX() != x + indent && textbuf[n] >= '0' && textbuf[n] <= '9' && textbuf[n - 1] == ' ')
			{
				char t[1024];
				int m;
				sscanf(textbuf + n, "%[^ ]%n", t, &m);
				t[m] = '\0';
				if (GetCurrentX() + TextStringWidth("|") + TextStringWidth(t) > x + w)
					break;
			}
			MovePen(GetCurrentX() + TextStringWidth(tempt), GetCurrentY());
			if (n == currentN)
			{
				cursorX = GetCurrentX() - indent / 4;
				cursorY = GetCurrentY();
			} 
			n++;
			tempt[0] = textbuf[n];
	...
	height = h * line;
	mySetPenColor(frameColor);
	drawRectangle(x, y  - (line - 1) * h, w, height, gs_textbox_color.fillflag);
	if( gs_textbox_color.fillflag ) {
		mySetPenColor( labelColor );
		drawRectangle(x, y  - (line - 1) * h, w, height, 0);
	}
	
	// show text
	mySetPenColor(labelColor);
	int i;
	for (i = 0, n = 0; i < line; i++)
	{
		MovePen(x + indent, textPosY - h * i);
		char tempt[2];
		tempt[1] = '\0';
		tempt[0] = textbuf[n];
		while (n < len && GetCurrentX() + TextStringWidth("|") + TextStringWidth(tempt) <= x + w)
	...
	if ( gs_UIState.kbdItem == id && (clock() >> 8) & 1) 
	{
		...
		//int cursorSize = GetPenSize();
		//SetPenSize(cursorSize + 10);
		DrawTextString("|");
		//SetPenSize(cursorSize);
	}
	...
	if ( gs_UIState.kbdItem == id )
	{
		...
		if ((gs_UIState.charInput >= 32 && gs_UIState.charInput < 127 || gs_UIState.charInput < 0) && len+1 < buflen ) {
			...
			textbuf[++len] = 0;*/
			int i;
			for (i = len - 1; i > currentN; i--)
			{
				textbuf[i + 1] = textbuf[i];
			}
			textbuf[++currentN] =  gs_UIState.charInput;
			textbuf[++len] = '\0';
